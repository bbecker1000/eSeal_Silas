...
# also want to look at pups and weaned pups together. weaned pups might move between beaches so we need to take a max counr from a single day each year
...
plot(LM_DB_cow)
# honestly this done a pretty ok job
...
plot(LM_SB_pup)
# honestly this done a pretty ok job, but could do better by dropping the one odd count from 1983
...
tidypupwnrNoT <- subset(tidyPups, Location != "All" & Age == "PUP&WNR")

#the following three models I don't know how to plot
gamm_cow <- gamm4(Count ~ s(Year), random=~(1|Location), data=tidycowsNoT)

gamm_pup <- gamm4(Count ~ s(Year), random=~(1|Location), data=tidypupNoT)

#gamm_pupwnr <- gamm4(Count ~ s(Year), random=~(1|Location), data=tidypupwnrNoT) #this one gives large eigenvalue warning

#also not sure how to plot these ones but here's a residual plot
ga_cow <- gam(Count ~ s(Year), data=tidyALLcowsest)
plot(fitted(ga_cow), resid(ga_cow))

ga_pup <- gam(Count ~s(Year), data=tidyALLpup)
plot(fitted(ga_pup), resid(ga_pup))

ga_pupwnr <- gam(Count ~s(Year), data=tidyALLpupwnr)
